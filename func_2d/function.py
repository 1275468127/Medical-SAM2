...
def train_sam(args, point_net,matcher, train_loader, criterion,optimizer, epoch, writer, model_ema=None):
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        ...
        #for ind, pack in enumerate(train_loader):
            ...
            loss_dict = criterion(outputs1, targets,epoch)
            ...
            mask = outputs1['pred_masks'] >= 1
            temp = eval_seg(mask, targets['gt_masks'].unsqueeze(0), threshold)
            total_eiou += temp[0]
            total_dice += temp[1]

            vis_image(imgs,mask, targets['gt_masks'].unsqueeze(0), os.path.join(args.path_helper['sample_path'], str(data_iter_step)+'epoch+' +str(epoch) + '.jpg'), reverse=False, points=None)

            pbar.update()

    return  tuple([total_eiou/len(train_loader) , total_dice/len(train_loader)])

def eval_point(args, point_net,matcher, train_loader, criterion,optimizer, epoch, writer, model_ema=None):
